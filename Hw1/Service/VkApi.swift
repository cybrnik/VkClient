...
class VkApi {
    ...
    class GetDataOperation: AsyncOperation {
        ...
        override func main() {
            ...
            requestToken = URLSession.shared.dataTask(with: url) { [weak self] data, _, _ in
                self?.data = data
                self?.finished()
            }
    ...
    class ParseDataOperation: Operation {
        ...
        override func main() {
            ...
            guard let operation = dependencies.first as? GetDataOperation,
    ...
    class SaveUsersDataToRealm: Operation {
        override func main() {
            guard let operation = dependencies.first as? ParseDataOperation else { return }
    ...
    func VKgetGroups(finished: @escaping () -> Void) {
        getGroupPromise().get { [self] data in
            parseGroupPromise(data: data).done { groups in 
